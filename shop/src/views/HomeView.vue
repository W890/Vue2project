    ...
    <div class="shop-swupe">
      <van-swipe class="my-swipe" :autoplay="3000" indicator-color="white">
        <van-swipe-item v-for="item in banner" :key="item.goods_id">
          <router-link :to="'/goods/' + item.goods_id"><img :src="item.image_url" alt="" /></router-link>
        </van-swipe-item>
      </van-swipe>
    </div>
    ...
    <div class="goods_list" v-for="item in categoryList" :key="item.goods_id">
      ...
      <div class="list">
        <van-grid :border="false" :column-num="3" :center="false" gutter="10">
          <van-grid-item v-for="item2 in item.goodsList" :key="item2.id" :to="'/goods/' + item2.id">
      ...
      <div class="foot-text">查看更多{{ item.name }}</div>
    </div>
    <br><br><br><br><br><br>
  </div>
</template>
